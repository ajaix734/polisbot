...
const sameIslandDLU = [
    {
        cs_time: moment.tz("2021-06-10 11:05:03", "Europe/Madrid").unix() - 1,
        anchors: [
            '{ "sword": 23, "archer": 31, "hoplite": 1, "rider": 4, "chariot": 1, "catapult": 1, "id": 9070, "type": "support", "town_id": ' + TOWN_MAP['Titan'] + ', "nl_init": true }',
            '{ "sword": 23, "archer": 31, "hoplite": 1, "rider": 4, "chariot": 1, "id": 9070, "type": "support", "town_id": ' + TOWN_MAP['Titan'] + ', "nl_init": true }'
        ]
    }
]
...
const diffIslandDLU1 = [
    {
        cs_time: moment.tz("2021-06-10 23:09:00", "Europe/Madrid").unix() - 1,
        anchors: [
            ...
            '{"hoplite": 110, "rider": 54, "small_transporter": 17, "attack_ship": 1, "id": 5105, "type": "support", "town_id": ' + TOWN_MAP['Titan 3'] + ', "nl_init": true }',
            '{"hoplite": 110, "rider": 54, "small_transporter": 17, "id": 5105, "type": "support", "town_id": ' + TOWN_MAP['Titan 3'] + ', "nl_init": true }'
        ]
    }
]
...
const bireme = [
    {
        cs_time: moment.tz("2021-06-11 10:21:00", "Europe/Madrid").unix() - 1,
        anchors: [
            '{ "bireme": 26, "demolition_ship": 1, "id": 5105, "type": "support", "town_id": ' + TOWN_MAP['Titan 9'] + ', "nl_init": true }',
            '{ "bireme": 26, "big_transporter": 1, "id": 5105, "type": "support", "town_id": ' + TOWN_MAP['Titan 9'] + ', "nl_init": true }',
            '{ "bireme": 26, "attack_ship": 1, "id": 5105, "type": "support", "town_id": ' + TOWN_MAP['Titan 9'] + ', "nl_init": true }',
            '{ "bireme": 26, "id": 5105, "type": "support", "town_id": ' + TOWN_MAP['Titan 9'] + ', "nl_init": true }'
        ]
    }
]
...
interval = setInterval(() => autoFarm(newToken), (600000 + (Math.random(5) * 60000)))
...
const cityBoosting = async (newToken, townToBoost, boostingStrategy) => {
    const getTowns = async (newToken, townToBoost) => {
        ...
        let towns = _.filter(result.data.json.towns, (town) => { return town.id !== parseInt(townToBoost) })
...
const autoFarm = async (newToken) => {
    let collect_url = `https://es98.grepolis.com/game/farm_town_overviews?town_id=9317&action=claim_loads_multiple&h=${newToken}`
    let payload = '{"towns":[5989,10610,7461,9317,9378,9067,4322,9984,5974,9754],"time_option_base":300,"time_option_booty":600,"claim_factor":"normal","town_id":9317,"nl_init":true}'
